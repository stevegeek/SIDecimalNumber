...
// Decimal size limits
SIDecimalMaxDigits                   =   38;
SIDecimalMaxExponent                 =  127;
SIDecimalMinExponent                 = -128;

// Scale for no Rounding
SIDecimalNoScale                     = 128
...
//Exceptions
SIDecimalNumberOverflowException     = @"SIDecimalNumberOverflowException";
SIDecimalNumberUnderflowException    = @"SIDecimalNumberUnderflowException";
SIDecimalNumberExactnessException    = @"SIDecimalNumberExactnessException";
SIDecimalNumberDivideByZeroException = @"SIDecimalNumberDivideByZeroException";
...
function SIDecimalMakeWithString(string, locale)
{
    ...
    if (inputlength > SIDecimalMaxDigits)
    {
        // input is too long, increase exponent and truncate
        exponent += inputlength - SIDecimalMaxDigits;
    }

    if (exponent > SIDecimalMaxExponent || exponent < SIDecimalMinExponent)
        return nil;
...
/*!
    @ingroup foundation
    Creates a SIDecimal object from a given matissa and exponent. The sign is taken from the sign of the mantissa. This cant do a full 34 digit mantissa representation as JS's 32 bits or 64bit binary FP numbers cant represent that. So use the SIDecimalMakeWithString if you want longer mantissa.
    @param mantissa the mantissa (though see above note)
    @param exponent the exponent
    @return A SIDecimal object, or nil on error.
*/
function SIDecimalMakeWithParts(mantissa, exponent)
{
    var m = [CPArray array],
        isNegative = NO;
    ...
    while((mantissa > 0) &&  ([m count] < SIDecimalMaxDigits)) // count selector here could be optimised away
    {
...
/*!
    @ingroup foundation
    Creates a SIDecimal 0.
    @return A SIDecimal object containing the value 0.
*/
function SIDecimalMakeZero()
{
    return SIDecimalMakeWithParts(0, 0);
}

/*!
    @ingroup foundation
    Creates a SIDecimal 1.
    @return A SIDecimal object containing the value 1.
*/
function SIDecimalMakeOne()
{
    return SIDecimalMakeWithParts(1, 0);
}


/*!
    @ingroup foundation
    Creates a SIDecimal NaN.
    @return A SIDecimal object containing the value NaN
*/
function SIDecimalMakeNaN()
{
    var d = SIDecimalMakeWithParts(0, 0);
    d._isNaN = YES;
    return d;
}
...
/*!
    @ingroup foundation
    Checks to see if a SIDecimal is zero. Can handle uncompacted strings.
    @return TRUE on zero.
*/
function SIDecimalIsZero(dcm)
{
...
/*!
    @ingroup foundation
    Checks to see if a SIDecimal is 1. Can handle uncompacted strings (it compacts them).
    @return TRUE on 1.
*/
function SIDecimalIsOne(dcm)
{
    SIDecimalCompact(dcm);
    // exponent doesnt matter as long as mantissa = 0
    if (!dcm._isNaN)
...
function _SIDecimalSetZero(result)
{
...
function _SIDecimalSetOne(result)
{
...
/*!
    @ingroup foundation
    Checks to see if a SIDecimal is Not A Number.
    @return TRUE on NaN.
*/
function SIDecimalIsNotANumber(dcm)
{
...
/*!
    @ingroup foundation
    Create a copy of a SIDecimal object
    @param dcm the SIDecimal number to copy.
    @return a new SIDecimal.
*/
function SIDecimalCopy(dcm)
{
...
/*!
    @ingroup foundation
    Compare two SIDecimal objects. Order is left to right (ie Ascending would
    mean left is smaller than right operand).
    @param leftOperand the left SIDecimal
    @param rightOperand the right SIDecimal
    @return CPOrderedAscending, CPOrderedDescending or CPOrderedSame.
*/
function SIDecimalCompare(leftOperand, rightOperand)
{
...
function _SimpleAdd(result, leftOperand, rightOperand, roundingMode, longMode)
{
    ...
    _SIDecimalSet(result, leftOperand);
    ...
    if (carry)
    {
        ...
        if (carry)
	    {
            ...
            if ((SIDecimalMaxDigits*factor) == [leftOperand._mantissa count])
	        {
                var scale = - result._exponent - 1;
                SIDecimalRound(result, result,scale,roundingMode);
	        }

	        if (SIDecimalMaxExponent < result._exponent)
	        {
	            result._isNaN = YES;
	            error = CPCalculationOverflow;
                result._exponent = SIDecimalMaxExponent;
	        }
	    }
    }
    return error;
...
/*!
    @ingroup foundation
    Performs the addition of 2 SIDecimal numbers.
    @param result the SIDecimal object in which to put the result
    @param leftOperand the left SIDecimal operand
    @param rightOperand the right SIDecimal operand
    @param roundingMode the rounding mode for the operation
    @return a CPCalculationError status value.
*/
function SIDecimalAdd(result, leftOperand, rightOperand, roundingMode, longMode)
{
    ...
    // check for zero
    if (SIDecimalIsZero(leftOperand))
    {
        _SIDecimalSet(result, rightOperand);
        return CPCalculationNoError;
    }
    if (SIDecimalIsZero(rightOperand))
    {
        _SIDecimalSet(result, leftOperand);
        return CPCalculationNoError;
    }

     var n1 = SIDecimalCopy(leftOperand),
         n2 = SIDecimalCopy(rightOperand);
    ...
    if (leftOperand._isNegative != rightOperand._isNegative)
    {
        ...
        {
	        n1._isNegative = NO;
	        return SIDecimalSubtract(result, rightOperand, n1, roundingMode);
	    }
        else
        {
	        n2._isNegative = NO;
	        return SIDecimalSubtract(result, leftOperand, n2, roundingMode);
	    }
    }

    var normerror = SIDecimalNormalize(n1, n2, roundingMode, longMode);
...
/*!
    @ingroup foundation
    Performs the subtraction of 2 SIDecimal numbers.
    @param result the SIDecimal object in which to put the result
    @param leftOperand the left SIDecimal operand
    @param rightOperand the right SIDecimal operand
    @param roundingMode the rounding mode for the operation
    @return a CPCalculationError status value.
*/
function SIDecimalSubtract(result, leftOperand, rightOperand, roundingMode)
{
    if (leftOperand._isNaN || rightOperand._isNaN)
    {
        result._isNaN = YES;
    ...
    if (leftOperand._isNegative != rightOperand._isNegative)
    {
        ...
        {
	        n1._isNegative = NO;
	        error1 = SIDecimalAdd(result, n1, rightOperand, roundingMode);
	        result._isNegative = YES;
        ...
        {
	        n2._isNegative = NO;
	        return SIDecimalAdd(result, leftOperand, n2, roundingMode);
	    }
    }

    var error = SIDecimalNormalize(n1, n2, roundingMode);
    var comp = SIDecimalCompare(leftOperand, rightOperand);

    if (comp == CPOrderedSame)
    {
        _SIDecimalSetZero(result);
        return CPCalculationNoError;
    }
...
{
    var error = CPCalculationNoError,
        n1 = SIDecimalMakeZero(),
        k = 0,
        firsttime = YES,
        stopk = SIDecimalMaxDigits+1,
    ...
    {
        while (CPOrderedAscending == SIDecimalCompare(n1, rightOperand))
        {
            if (stopk == k)
                break;
	        if (n1._exponent)
	        ...
	        else
	        {
                ...
                {
                    if (result._exponent == SIDecimalMinExponent)
                    {
        ...
        // k == (SIDecimalMaxDigits+1) then we should stop i.e. last subtract
        // was last valid one.
        if (stopk == k)
        {
        ...
        error1 = SIDecimalSubtract(n1, n1, rightOperand, roundingMode);
        if (error1 != CPCalculationNoError)
            error = error1;
...
/*!
    @ingroup foundation
    Performs a division of 2 SIDecimal numbers.
    @param result the SIDecimal object in which to put the result
    @param leftOperand the left SIDecimal operand
    @param rightOperand the right SIDecimal operand
    @param roundingMode the rounding mode for the operation
    @return a CPCalculationError status value.
*/
function SIDecimalDivide(result, leftOperand, rightOperand, roundingMode)
{
    var error = CPCalculationNoError,
    ...
    if (result._exponent + exp > SIDecimalMaxExponent)
    {
        result._isNaN = YES;
        if (neg)
    ...
    else if (result._exponent + exp < SIDecimalMinExponent)
    {
        // We must cut off some digits
        SIDecimalRound(result, result, exp + SIDecimalMaxExponent + 1, roundingMode);
        error = CPCalculationLossOfPrecision;

        if (result._exponent + exp < SIDecimalMinExponent)
        {
	        SIDecimalSetZero(result);
	        return error;
        }
    }
...
function _SimpleMultiply(result, leftOperand, rightOperand, roundingMode, powerMode)
{
    ...
    for (; i < [rightOperand._mantissa count]; i++)
    {
        ...
        error1 = SIDecimalAdd(result, result, n, roundingMode, YES);
    ...
    // perform round to SIDecimalMaxDigits
    if ([result._mantissa count] > SIDecimalMaxDigits && !powerMode)
    {
        result._isCompact = NO;
        var scale = SIDecimalMaxDigits - ([result._mantissa count] + result._exponent);
        SIDecimalRound(result, result, scale ,roundingMode); // calls compact
...
/*!
    @ingroup foundation
    Performs multiplication of 2 SIDecimal numbers.
    @param result the SIDecimal object in which to put the result
    @param leftOperand the left SIDecimal operand
    @param rightOperand the right SIDecimal operand
    @param roundingMode the rounding mode for the operation
    @return a CPCalculationError status value.
*/
function SIDecimalMultiply(result, leftOperand, rightOperand, roundingMode, powerMode)
{
    var error = CPCalculationNoError,
        exp = leftOperand._exponent + rightOperand._exponent,
        neg = (leftOperand._isNegative != rightOperand._isNegative);
    ...
    // check for zero
    if (SIDecimalIsZero(rightOperand) || SIDecimalIsZero(leftOperand))
    {
        _SIDecimalSetZero(result);
        return CPCalculationNoError;
    }
...
/*!
    @ingroup foundation
    Raises a SIDecimal number to a power of 10.
    @param result the SIDecimal object in which to put the result
    @param dcm the SIDecimal operand
    @param power the power to raise to
    @param roundingMode the rounding mode for the operation
    @return a CPCalculationError status value.
*/
function SIDecimalMultiplyByPowerOf10(result, dcm, power, roundingMode)
{
    _SIDecimalSet(result, dcm);
    var p = result._exponent + power;
    if (p > SIDecimalMaxExponent)
    {
        result._isNaN = YES;
        return CPCalculationOverflow;
    }
    if (p < SIDecimalMinExponent)
    {
        result._isNaN = YES;
        return CPCalculationUnderflow;
    }
...
/*!
    @ingroup foundation
    Raises a SIDecimal number to the given power.
    @param result the SIDecimal object in which to put the result
    @param dcm the SIDecimal operand
    @param power the power to raise to
    @param roundingMode the rounding mode for the operation
    @return a CPCalculationError status value.
*/
function SIDecimalPower(result, dcm, power, roundingMode)
{
    var error = CPCalculationNoError,
        neg = (dcm._isNegative && (power % 2)),
        n1 = SIDecimalCopy(dcm);

    n1._isNegative = NO;

    _SIDecimalSetOne(result);
    ...
    while (e)
    {
        if (e & 1)
        {
	        error = SIDecimalMultiply(result, result, n1, roundingMode); //, YES); // enable for high precision powers
        }

        error = SIDecimalMultiply(n1, n1, n1, roundingMode); //, YES); // enable for high precision powers
...
/*  // enable is powerMode to do finally rounding to Max Digits.
    if ([result._mantissa count] > SIDecimalMaxDigits)
    {
        result._isCompact = NO;
        var scale = SIDecimalMaxDigits - ([result._mantissa count] + result._exponent);
        SIDecimalRound(result, result, scale ,roundingMode); // calls compact
...
/*!
    @ingroup foundation
    Normalises 2 SIDecimals. Normalisation is the process of modifying a
    numbers manitssa to ensure that both SIDecimals have the same exponent.
    @param dcm1 the first SIDecimal
    @param dcm2 the second SIDecimal
    @param roundingMode the rounding mode for the operation
    @return a CPCalculationError status value.
*/
function SIDecimalNormalize(dcm1, dcm2, roundingMode, longMode)
{
    var factor = (longMode)?2:1;
    ...
    // ensure compact
    if(!dcm1._isCompact)
        SIDecimalCompact(dcm1);
    if(!dcm2._isCompact)
        SIDecimalCompact(dcm2);
    ...
    if (e2 > e1)
        l = MIN((SIDecimalMaxDigits*factor) - l2, e); //(e2 - e1));
    else
        l = MIN((SIDecimalMaxDigits*factor) - l1, e); //(e1 - e2));
    ...
    if (l != ABS(e2 - e1))//e2 - e1)
    {
        ...
        if( e2 > e1 )
        {
            SIDecimalRound(dcm1, dcm1, -dcm2._exponent, roundingMode);
            l1 = SIDecimalIsZero(dcm1);
        }
        else
        {
            SIDecimalRound(dcm2, dcm2, -dcm1._exponent, roundingMode);
            l2 = SIDecimalIsZero(dcm2);
        }
        ...
        if ((dcm1._exponent != dcm2._exponent) && ((!l1) || (!l2)) )
	    {
            ...
            {
                l1 = [dcm1._mantissa count];
                l = MIN((SIDecimalMaxDigits*factor) - l1, ABS(dcm1._exponent - dcm2._exponent));
                for (i = 0; i < l; i++)
                {
                ...
                l = MIN((SIDecimalMaxDigits*factor) - l2, ABS(dcm2._exponent - dcm1._exponent));
                for (i = 0; i < l; i++)
                {
...
/*!
    @ingroup foundation
    Rounds a SIDecimal off at a given decimal position. scale specifies the
    ...
    @return a CPCalculationError status value.
*/
function SIDecimalRound(result, dcm, scale ,roundingMode)
{
    if (dcm._isNaN)
        return;
    ...
    else if (l <= 0)
    {
        ...
        if (up)
        {
	        ...
	        if (i == -1)
	        {
	            ...
	            if (result._exponent >= SIDecimalMaxExponent)
	            {
...
/*!
    ...
    @param dcm the SIDecimal operand
*/
function SIDecimalCompact(dcm)
{
    // if positive or zero exp leading zeros simply delete, trailing ones u need to increment exponent
    if (!dcm || [dcm._mantissa count] == 0 || SIDecimalIsNotANumber(dcm) )
    	return;


    if( SIDecimalIsZero(dcm) )
    {
        // handle zero number compacting
        _SIDecimalSetZero(dcm);
        return;
    }
    ...
    while( [dcm._mantissa lastObject] === 0 )
    {
        ...
        dcm._exponent++;
        if (dcm._exponent+1 > SIDecimalMaxExponent)
        {
...
/*!
    @ingroup foundation
    Convert a SIDecimal to a string representation.
    @param dcm the SIDecimal operand
    @param locale the locale to use for the conversion
    @return a CPString
*/
function SIDecimalString(dcm, locale)
{
 ...
 /*
    ...
    var sep = 0;
    if ((locale == nil) || (sep = [locale objectForKey: SIDecimalSeparator]) == nil)
        sep = @".";
